...
def _triage_item(item: dict[str, Any]) -> TriageDecision:
    ...
    upw = _unpaywall_info(doi)
    upw_licenses: list[str] = []
    oa_status = str(upw.get("oa_status", ""))
    source_url = data.get("url") or str(upw.get("doi_url", ""))
    ...
    if upw.get("best_oa_location", {}).get("url_for_pdf"):
        source_url = str(upw["best_oa_location"]["url_for_pdf"])
...
def train_lora_on_modal(
    dataset_repo: str,
    model_repo: str,
    base_model: str = "unsloth/gemma-4-12b-it",
    max_steps: int = 20,
